...
The analyzer RF span for this module is: \( RF_{SA} = 0\text{–}2.9\ \text{GHz} \)
 
### 1st LO from analyzer

The analyzer 1st LO is:  \( LO_{SA} = RF_{SA} + IF_1 \)  with \( IF_1 = 3.9107\ \text{GHz} \)

So: \( LO_{SA} = RF_{SA} + 3.9107\ \text{GHz} \)

Range: \( LO_{SA} \approx 3.9\text{–}6.8\ \text{GHz} \)

### TG LO (MAX2870)

The MAX2870 is fixed at: \( f_{\text{LO,TG}} = IF_1 = 3.9107\ \text{GHz} \)
...
Difference product: \( IF_{TG} = RF_{\text{mixer}} - LO_{\text{mixer}} \)
...
- SPI control from Raspberry Pi MCU to configure the fixed 3.9107 GHz LO  
...
- FIne tune attenuator pads for: SA 1st LO, MAX2870 LO, HMC219 IF/RF, and final TG output.  
- 3 GHz LPF creates sufficient rejection of mixer images and LO feedthrough.
